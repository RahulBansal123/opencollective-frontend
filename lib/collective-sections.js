...
import { MainSections, MainSectionsKeys, MainSectionsNames, Sections } from '../components/collective-page/_constants';
...
const getCollectiveSectionsFromAPI = collective => {
  const features = get(collective, 'features');
  return pickBy(features, feature => ['ACTIVE', 'AVAILABLE', 'DISABLED'].includes(feature));
};

// const filterFeaturesByUser = (features, collective, isAdmin, isHostAdmin) => {
//   let features;
//   if (!isAdmin && !isHostAdmin) {
...
export const getFilteredSectionsForCollective = (collective, isAdmin, isHostAdmin) => {
  ...
  const featuresForCollective = getCollectiveSectionsFromAPI(collective);
  console.log('supported features for collective', featuresForCollective);

  let sections = Object.keys(featuresForCollective);
  // return MAIN sections
  let mainSections = sections.filter(section => MainSectionsKeys.includes(section));
