...
export const getDefaultSectionsForCollective = (type, isActive) => {
  ...
  if (hasNewCollectiveNavbar) {
    defaultSections = DEFAULT_SECTIONS_NEW_NAVIGATION[typeKey] || [];
  } else {
    defaultSections = DEFAULT_SECTIONS[typeKey] || [];
  }
  const toRemove = new Set();

  // remove this line and calculate elsewhere
...
// const filterInactiveSections
// const filterSectionsBasedOnUserRole
...
//   // New Connect section, don't display if:
...
export const filterSectionsByData = (sections, collective, isAdmin, isHostAdmin) => {
  ...
  // hide from non admins to double check
  if (!isAdmin && !isHostAdmin) {
    // const { updates, transactions, balance } = collective.stats || {};
    // const expenses = collective.expenses || [];
    // if (!updates) {
...
export const getFilteredSectionsForCollective = (collective, isAdmin, isHostAdmin) => {
  ...
  // first get sections/features from the API
  const featuresForCollective = get(collective, 'features');
  console.log(`features for collective type ${collective.type} from API: `, featuresForCollective);
  // remove UNSUPPORTED
  let filteredFeaturesForCollective = pickBy(featuresForCollective, feature =>
    ['ACTIVE', 'AVAILABLE', 'DISABLED'].includes(feature),
  );
  console.log('filter out Unsupported Features For Collective', filteredFeaturesForCollective);

  if (!isAdmin && !isHostAdmin) {
    filteredFeaturesForCollective = pickBy(featuresForCollective, feature => ['ACTIVE'].includes(feature));
    const object = pickBy(featuresForCollective, feature => ['ACTIVE'].includes(feature));
    console.log('obj', object);
  }

  console.log('filter out from non admins', filteredFeaturesForCollective);

  let sections = Object.keys(filteredFeaturesForCollective);

  sections = sections.map(section => {
    return Sections[section];
  });

  sections = sections.filter(s => !isUndefined(s));

  //sections = filterSectionsForUser(sections, isAdmin, isHostAdmin);

  //sections = filterSectionsByData(sections, collective, isAdmin, isHostAdmin);

  console.log('Filtered sections to render', sections);
