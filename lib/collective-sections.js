...
import { get, isUndefined, omit, pickBy } from 'lodash';

import { NavbarSectionsKeys, Sections } from '../components/collective-page/_constants';
...
import { LessThanDimensions } from '@styled-icons/fa-solid/LessThan';
...
// Define default sections based on collective type
// const DEFAULT_SECTIONS_NEW_NAVIGATION = {
//   [CollectiveType.ORGANIZATION]: [
//     Sections.CONTRIBUTE,
//     Sections.CONTRIBUTIONS,
//     Sections.EVENTS,
...
export const filterSectionsForUser = (collectiveSections, isAdmin, isHostAdmin) => {
  ...
  console.log('filterSectionsForUser', sections);
...
// Navigation v2
export const getDefaultSectionsForCollectiveV2 = collective => {
  const features = get(collective, 'features');
  return pickBy(features, feature => ['ACTIVE', 'AVAILABLE', 'DISABLED'].includes(feature));
};

export const getNavbarSectionsForCollective = collective => {
  if (hasNewCollectiveNavbar) {
    // first get supported sections/features from the API
    const featuresForCollective = getDefaultSectionsForCollectiveV2(collective);
    console.log('api features for collective', featuresForCollective);

    let sections = Object.keys(featuresForCollective);

    // return MAIN/listed on navbar menu sections
    const navbarSections = sections.filter(section => NavbarSectionsKeys.includes(section));

    sections = navbarSections.map(section => {
...
export const getFilteredSectionsForCollectiveV2 = (collective, isAdmin, isHostAdmin) => {
  // 1. get all SUPPORTED (ACTIVE, AVAILABLE, DISABLED) features for collective
  let featuresForCollective = getDefaultSectionsForCollectiveV2(collective);
  console.log('api features for collective', featuresForCollective);
  // 2. hide DISABLED and AVAILABLE features from non-admins
  if (!isAdmin && !isHostAdmin) {
    featuresForCollective = pickBy(featuresForCollective, feature => ['ACTIVE'].includes(feature));
  }
  console.log('remove disabled and available', Object.keys(featuresForCollective));
  let featuresForCollectiveArray = Object.keys(featuresForCollective);
  // 3. remove these standalone sections
  featuresForCollectiveArray = featuresForCollectiveArray.map(section => {
    return Sections[section];
  });
  featuresForCollectiveArray = featuresForCollectiveArray.filter(
    feature => !['updates', 'conversations', 'recurring-contributions', 'goals'].includes(feature),
  );
  console.log('v2 sections', featuresForCollectiveArray);
  return featuresForCollectiveArray;
};
...
export const getFilteredSectionsForCollective = (collective, isAdmin, isHostAdmin) => {
  // v2
  // IGNORE settings.collectivePage.settings in v2, this will stem from FEATURES endpoint soon
  if (hasNewCollectiveNavbar) {
    const featuresForCollectiveV2 = getFilteredSectionsForCollectiveV2(collective, isAdmin, isHostAdmin);
    return featuresForCollectiveV2;
    // end v2
  } else {
